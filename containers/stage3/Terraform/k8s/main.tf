...
# Create a k8s secret to manage the db credentials
resource "kubernetes_secret" "db-credentials" {
  metadata {
    name      = "db-credentials"
  ...
  data = {
    username = base64encode(local.db_creds.username)
    password = base64encode(local.db_creds.password)
  }
}

# Define configmap for CWA
